...
//
// Calculo de raizes reais de funcoes reais – metodo Ponto Falso
...
// #define f(x) (pow(E, x) - x - 2)
// #define f(x) (pow(x, 3) - 9*x + 3)

int main()
{
    int k = 0, nIterations = 0;
    long double a, b, fa, fb, x, fx, oldX, precision1, precision2, test, errorPercentage, minError;
    ...
    if (fa * fb < 0)
    {
        ...
        {
            if (fabsl(fa) < precision2)
            {
                x = a;
                break;
            }
            if (fabsl(fb) < precision2)
            {
                x = b;
                break;
            }
            ...
            oldX = x;
            x = (a * fabsl(fb) + b * fabsl(fa)) / (fabsl(fb) + fabsl(fa));
            fx = f(x);
            ...
            if(minError >= errorPercentage){
                printf("\nPorcentagem de erro minimo atingido.\nporcentagem de erro procurado:%Lf\nporcentagem de erro atual: %Lf\n",minError, errorPercentage);
                break;
            }
        }
